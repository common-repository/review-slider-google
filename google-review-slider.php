...
function review_slider_get_reviews(){
  $APIkey = 'AIzaSyB4d6GlOpw2fRuRB1DnpG049194c4OnR1Y';
  ...
  $address_components = $raw_results->result->address_components;
  for ($i = 0; $i < sizeof($address_components); $i++){
    if(in_array("locality", $address_components[$i]->types)){
      update_option( 'review_slider_city', $raw_results->result->address_components[$i]->long_name);
      break;
    }
  }
}
    ...
    for($i = 0; $i < sizeof($reviews); $i++){
      ...
      $slider_html .=
        '<div class="review-slider-review-container">' .
          '<div class="review-slider-star-container" style="color: ' . $secondary_color . ';">';
      ...
      if($city)
        ...
        $author_text = $reviews[$i]->author_name;
